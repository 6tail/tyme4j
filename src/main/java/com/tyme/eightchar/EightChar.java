...
public class EightChar extends AbstractCulture {
    ...
    public List<SolarTime> getSolarTimes(int startYear, int endYear) {
        return getSolarTimes(startYear, endYear, false);
    }
     ...
     * @param startYear  开始年(含)，支持1-9999年
     * @param endYear    结束年(含)，支持1-9999年
     * @param ignoreHour 忽略时辰
     * @return 公历时刻列表
     */
    public List<SolarTime> getSolarTimes(int startYear, int endYear, boolean ignoreHour) {
        ...
        while (y <= endYear) {
            if (y >= baseYear) {
                ...
                if (solarTime.getYear() >= startYear) {
                    ...
                    // 验证一下
                    if (ignoreHour) {
                        if (time.getLunarHour().getYearSixtyCycle().getIndex() == getYear().getIndex() && time.getLunarHour().getMonthSixtyCycle().getIndex() == getMonth().getIndex() && time.getLunarHour().getDaySixtyCycle().getIndex() == day.getIndex()) {
                            l.add(time);
                        }
                    } else {
